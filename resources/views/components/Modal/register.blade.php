<div class="hidden modal overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none justify-center items-center" id="registerModal" >
    ...
    <div class="border-0 rounded-xl shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
        ...
        <p class="text-gray-400 mt-1 text-sm">
            Join Aztech and start your real project
        </p>
        </div>
        <form action="{{ route('register') }}" method="POST">
            ...
            <div class="relative p-6 flex-auto mx-10">
                ...
                <div>
                    ...
                    <input class="appearance-none border border-gray-300 rounded-lg w-full py-3 px-4 placeholder-serv-text text-xs mb-3" id="password_confirmation" name="password_confirmation" type="password" placeholder="At least 8 characters" required>

                    @if($errors->has('password_confirm'))
                        <p class="text-red-500 mb-3 text-sm">{{ $errors->first('password_confirm') }}</p>
                    @endif
                </div>
